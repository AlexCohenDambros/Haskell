...
contaCaractere string character = length [n | n <- string, n == character]
...
dobroNaoNegativo lista = [n^2 | n <- lista]
...
pitagoras valor = [(a,b,c) | a <-[1..valor], b <-[1..valor], c <- [1..valor], ((a^2) + (b^2)) == (c^2)]
...
primeirosPrimos valor = [n | n <- [2..valor], length((divisoresden n)) == 2]
